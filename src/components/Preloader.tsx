...
const Preloader: React.FC<{ onComplete: () => void }> = ({ onComplete }) => {
  ...
  useEffect(() => {
    ...
    style.textContent = `
      .orange-stroke {
        animation: fillAnimation 1.2s ease-in-out forwards;
      }
    ...
    // Reduce preloader time to 1 second for better FCP - allow content to render faster
    const timer = setTimeout(() => {
      setIsLoaded(true);
    }, 1000); // Reduced to 1s for optimal performance
